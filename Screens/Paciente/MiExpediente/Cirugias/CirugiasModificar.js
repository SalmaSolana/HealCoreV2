...
import { VStack, HStack, Menu, Pressable, HamburgerIcon, Heading, FormControl, Select, CheckIcon, Input, Button, Text, NativeBaseProvider, Center, Box, StatusBar, Image, ScrollView } from "native-base";
      ...
      <Box borderWidth={3} borderRadius="xl" borderColor="info.800" alignSelf="center" paddingTop={4} bg="white" marginTop={4} width="95%" paddingBottom={10}>
        ...
        <VStack marginX={30} space={4}>
          ...
          <HStack space={2} alignItems="center" marginTop={10}>
            ...
            <Input w="80%" h="100" multiline numberOfLines={3} fontStyle={"italic"} bg={"lightBlue.200"} fontWeight={"bold"} color="black" fontSize={"2xl"} placeholder="Extracción de Vesicula Biliar" placeholderTextColor={"black"}></Input>
          </HStack>
          {/*Tipo */}
          <HStack space={2} alignItems={"center"}>
            <Text fontWeight={"bold"} fontSize="xl">Tipo:</Text>
            <FormControl w="30%">
              <Select accessibilityLabel="Choose Service" placeholder='Urgente' placeholderTextColor={"black"} fontWeight={"bold"} fontSize={"lg"} _selectedItem={{ bg: "black", endIcon: <CheckIcon size={3} /> }}>
                <Select.Item label="Programada" value="ux" />
                <Select.Item label="Urgente" value="web" />
                <Select.Item label="Ambulantoria" value="cross" />
              </Select>
            </FormControl>
          </HStack>
          {/*Fecha*/}
...
export default function CirugiasModificar({ navigation }) {
  return (
    <NativeBaseProvider>
      <AppBar />
      <Cir />
